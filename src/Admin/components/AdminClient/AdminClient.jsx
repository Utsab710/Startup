...
function AdminClient() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name || (!formData.image && !editingClient)) {
    ...
    try {
      if (editingClient) {
        ...
        const response = await axios.put(
          `http://localhost:8000/api/client/update-Clientlogo/${editingClient._id}`,
          data,
          { withCredentials: true }
        );
        setClients(
          clients.map((c) => (c._id === editingClient._id ? response.data : c))
        );
  ...
  const handleDelete = async (clientId) => {
    ...
    try {
      ...
      if (editingClient && editingClient._id === clientId) {
        handleCancelEdit();
      }
    } catch (err) {
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-10">
          <h1 className="text-4xl font-extrabold text-gray-900 tracking-tight sm:text-5xl">
            <span className="text-indigo-600">Clients</span> Management
          </h1>
          <p className="mt-3 max-w-2xl mx-auto text-xl text-gray-500 sm:mt-4">
            Add and manage client logos for your website
          </p>
        </div>
        ...
        {/* Form Section */}
        <div className="mb-16" id="client-form">
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            <div className="px-4 py-5 sm:p-6">
              <h2 className="text-2xl font-bold text-gray-900 mb-8 flex items-center">
                ...
                {editingClient ? "Update Client Logo" : "Add New Client"}
              </h2>
